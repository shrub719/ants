...
class Simulation():
    ...
    def GetAreaDetails(self, StartRow, StartColumn, EndRow, EndColumn):
        if EndRow > self._NumberOfRows:
            EndRow = self._NumberOfRows
            print("End row out of bounds; set to max")

        if EndColumn > self._NumberOfColumns:
            EndColumn = self._NumberOfColumns
            print("End column out of bounds; set to max")
