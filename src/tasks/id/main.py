...
class Simulation():
    ...
    def SetUpANestAt(self, Row, Column):
        ...
        self._Ants.append(QueenAnt(Row, Column, Row, Column, self._Ants))
        for Worker in range(2, self._StartingAntsInNest + 1):
            self._Ants.append(WorkerAnt(Row, Column, Row, Column, self._Ants))
...
class Ant(Entity):
    ...
    def __init__(self, StartRow, StartColumn, NestInRow, NestInColumn, Ants):
        super().__init__(StartRow, StartColumn)
        ...
        id = ""
        valid = False
        ids = {ant.CoolerID for ant in Ants}
        while not valid or id in ids:
            id = input("Enter ID: ")
            valid = (
                len(id) == 8
                and id[:2] == id[:2].upper()
                and id[:2].isalpha()
                and id[2:].isnumeric()
                and sum(int(digit) for digit in id[2:]) % 2 == 0
            )

        self.CoolerID = id
        print("Created Ant with id", id)
...
class QueenAnt(Ant):
    def __init__(self, StartRow, StartColumn, NestInRow, NestInColumn, Ants):
        super().__init__(StartRow, StartColumn, NestInRow, NestInColumn, Ants)
        ...
        if self._FoodLevel < AntsInNestCount * 5:
            ...
            for A in range(1, self._NumberOfQueens + 1):
                ...
                if RNo1 < 50:
                    ...
                    if RNo2 < 2:
                        Ants.append(QueenAnt(self._Row, self._Column, self._Row, self._Column, Ants))
                        self._NumberOfQueens += 1
                    else:
                        Ants.append(WorkerAnt(self._Row, self._Column, self._Row, self._Column, Ants))
