...
def Main():
    ...
    while Choice != "9":
        ...
        if Choice == "1":
            ...
            ThisSimulation.AddFoodToCellManually()
    input()
...
def DisplayMenu():
    ...
    print("6. Add food to specific location")
    print("9. Quit")
    print()
    print("> ", end='')
...
class Simulation():
    ...
    def AddFoodToCellManually(self):
        row, column = GetCellReference()
        index = self.__GetIndex(row, column)
        cell = self._Grid[index]

        if self.GetNestInCell(cell):
            print("Cannot add food to a cell with a nest")
            return

        amount = int(input("Enter amount of food: "))
        if amount < 0:
            print("Amount must be a positive integer")
            return
        
        self.AddFoodToCell(row, column, amount)
