    ...
    def SetUpANestAt(self, Row, Column):
        ...
        for Worker in range(2, self._StartingAntsInNest + 1):
            if random.randint(0, 20) < 3:
                self._Ants.append(LazyAnt(Row, Column, Row, Column))
            else:
...
class LazyAnt(WorkerAnt):
    def __init__(self, StartRow, StartColumn, NestInRow, NestInColumn):
        super().__init__(StartColumn, StartColumn, NestInRow, NestInColumn)
        self._TypeOfAnt = "lazy"
        self._FoodCapacity = 30
        print("Created lazy ant")
    
    def ChooseCellToMoveTo(self, ListOfNeighbours, IndexOfNeighbourWithStrongestPheromone):
        if random.randint(0, 1) == 0:
            print("Lazy ant moved")
            super().ChooseCellToMoveTo(ListOfNeighbours, IndexOfNeighbourWithStrongestPheromone)
