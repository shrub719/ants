...
class Simulation():
    ...
    def IsAntNear(self, Row, Column):
        for Index in self.__GetIndicesOfNeighbours(Row, Column):
            if self.GetNumberOfAntsInCell(self._Grid[Index]) > 0:
                return True
...
class SoldierAnt(Ant):
    ...
    def HasFriends(self, Simulation):
        return Simulation.IsAntNear(self._Row, self._Column)
